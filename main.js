...
const mathFunctions = [
  ...
  },
  {
    ...
    calc: (index, maxIndex, maxMargin) =>
      Math.pow(index / maxIndex, 2) * maxMargin,
  },
  {
    name: "squareRoot",
    calc: (index, maxIndex, maxMargin) =>
      Math.sqrt(index / maxIndex) * maxMargin,
  },
  {
    name: "cubic",
    calc: (index, maxIndex, maxMargin) =>
      Math.pow(index / maxIndex, 3) * maxMargin,
  },
  {
    name: "logarithmic",
    calc: (index, maxIndex, maxMargin) => {
  ...
  },
  {
    ...
    calc: (index, maxIndex, maxMargin) => {
      ...
      const sinePart =
        ((Math.sin((index / maxIndex) * Math.PI - Math.PI / 2) + 1) / 2) * 0.3;
  ...
  },
  {
    name: "reverseLinear",
    calc: (index, maxIndex, maxMargin) =>
      maxMargin - (index / maxIndex) * maxMargin,
  },
  {
    name: "reverseQuadratic",
    calc: (index, maxIndex, maxMargin) =>
      maxMargin - Math.pow(index / maxIndex, 2) * maxMargin,
  },
  {
    name: "reverseCubic",
    calc: (index, maxIndex, maxMargin) =>
      maxMargin - Math.pow(index / maxIndex, 3) * maxMargin,
  },
  {
    name: "wave",
    calc: (index, maxIndex, maxMargin) => {
      ...
      return (
        (Math.sin((index / maxIndex) * Math.PI * waves) * 0.5 + 0.5) * maxMargin
      );
    },
  },
  {
...
function animateContainers() {
  moveContainers.forEach((container) => {
    let lines = container.querySelectorAll("p, div, h2, h3");
    const selectedFunc =
      mathFunctions[Math.floor(Math.random() * mathFunctions.length)];
...
window.addEventListener("load", () => {
  ...
  console.log(imprintContent);
  

  imprintBtn.addEventListener("click", () => {
    imprintContent.classList.toggle("hidden");
  });
});
